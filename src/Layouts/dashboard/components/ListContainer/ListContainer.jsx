...
const ListContainer = () => {
  ...
  const current_page = useSelector((state) => state.other.current_page);
      ...
      buttons: [
        {
          ...
          onClick: () => {
            ...
            dispatch(deleteFolder(formData))
              .then((result) => {
  ...
  const getAllDataList = () => {
    if (type === "Outlook" && current_state === "Outlook") {
      console.log("Actin HIT");
      dispatch(getListData(selectedItem?.folder_id, current_page))
        .then((result) => {
      ...
      dispatch(getListDataGoogle(selectedItem?.folder_id, current_page))
        .then((result) => {
  ...
  useEffect(() => {
    if (sourceValue === "Outlook") {
      ...
      if (codeParam) {
        const formData = new FormData();
        formData.append("code", codeParam);
        dispatch(authenticateGoogle(formData))
          .then((result) => {
  ...
  return (
    <>
            ...
            current_state == "Outlook"
              ? data.folders.map((val, ind) => (
                  <ListItem
                    ...
                    sx={{
                      backgroundColor:
                        selectedItem?.folder_name === val?.folder_name
                          ? "#B5DCFF"
                          : "transparent" || "",
                      height: "35px",
                      cursor: "pointer",
              ...
              : type === "Google" &&
                data_google != "undefined" &&
                data_google?.length > 0 &&
      ...
      {loading && <Loading title="Deleting please wait..." open={true} />}
      <ListDataContainer
        data={listData}
        type={type}
        group={group}
