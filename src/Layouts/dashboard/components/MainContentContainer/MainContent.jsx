...
const MainContent = () => {
  ...
  const handleSnoozeSubmit = () => {
    ...
    dispatch(
      type === "Outlook" ? snoozeEmail(formData) : snoozeEmailGoogle(formData)
    )
      .then((result) => {
        console.log(result);
        setLoadingM(false);
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(
      type === "Outlook" ? replyMail(formData) : replyMailGoogle(formData)
    )
      .then((result) => {
        setloading(false);
        enqueueSnackbar(result.data.message, {
  ...
  const handleMenuItemClick = (data) => {
    ...
    if (data.title === "Snooze") {
      ...
      if (type === "Google") {
        ...
        dispatch(unFlagEmailGoogle(body))
          .then((result) => {
            setLoadingM(false);
      ...
      setMoveDialg(true);
    } else if (data.title === "Copy") {
      setCopyDialog(true);
    }
  };
  const getMailThreads = () => {
    if (type === "Google") {
      dispatch(getGoogleThreads(content?.mail_id))
        .then((result) => {
      ...
      dispatch(getOutlookThreads(content?.conversationId))
        .then((result) => {
  ...
  const handleFavoriteClick = (val) => {
    ...
    if (val === "FAV") {
      if (type === "Google") {
        dispatch(flagEmailGoogle(body2))
          .then((result) => {
            // sweetalertFunc("Email flagged Successfully")
          })
          .catch((err) => {
      ...
      if (type === "Google") {
        const formDataG = new FormData();
        formDataG.append("id", content.id);
        dispatch(unFlagEmailGoogle(body2))
          .then((result) => {
            // sweetalertFunc("Removed from favorites Successfully")
          })
          .catch((err) => {
                      ...
                      >
                        <IconButton>
                          {isFavorite ? (
                            <FavoriteIcon
                              sx={{ color: "#040263" }}
                              onClick={() => handleFavoriteClick("UNFAV")}
                            />
                          ) : (
                            <FavoriteBorderIcon
...
<div
      ...
      <Dialog open={snoozeDialogOpen} onClose={handleSnoozeDialogClose}>
        ...
        <DialogActions>
          ...
          <Button onClick={handleSnoozeSubmit} color="primary">
            Submit
          </Button>
        </DialogActions>
      </Dialog>
